...
int main(int argc, char* argv[]) {
  ...
  cv::resize(left, left, left.size() / 4 * 4);
  cv::resize(right, right, right.size() / 4 * 4);
  ...
  switch (left.type()) {
    ...
    case CV_16UC1:
      ...
      break;
    default:
      std::cerr << "invalid input image color format" << left.type()
                << std::endl;
      std::exit(EXIT_FAILURE);
  }

  sgm::StereoSGM ssgm(left.cols, left.rows, disp_size, bits, 8,
  ...
  // show image
  cv::imshow("image", output * 256 / disp_size * 2);
  cv::imshow("left", left);
